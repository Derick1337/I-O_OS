...
// --- CLASSE Menos Recentemente Usada (MRU) ---
class MRU
{
protected:
    int num_frames;
    std::vector<int> frames;
    std::map<int, int> last_used_time;
    int time_counter;
    int page_replacements;

public:
    MRU(int n_frames) : num_frames(n_frames), time_counter(0), page_replacements(0) {}
    int get_page_replacements() const { return page_replacements; }
...
protected:
    ...
    void replace_page(int page)
    {
        ...
        int MRU_page = frames[0];
        int min_time = last_used_time.at(MRU_page);
        int victim_index = 0;

        for (size_t i = 1; i < frames.size(); ++i)
        {
            int current_page_in_frame = frames[i];
            if (last_used_time.at(current_page_in_frame) < min_time)
            {
                min_time = last_used_time.at(current_page_in_frame);
                MRU_page = current_page_in_frame;
                victim_index = i;
            }
        }

        frames[victim_index] = page;
        last_used_time.erase(MRU_page);
        page_replacements++;
    }
...
public:
    void execute(const std::vector<int> &access_sequence)
    {
        for (int page : access_sequence)
        {
            ...
            last_used_time[page] = time_counter;
        }
    }
};

// --- CLASSE NÃO USADA FREQUENTEMENTE (NUF) ---
class NUF
{
protected:
    int num_frames;
...
public:
    NUF(int n_frames) : num_frames(n_frames), page_replacements(0) {}
    int get_page_replacements() const { return page_replacements; }
...
protected:
    ...
    void replace_page(int page)
    {
        ...
        int min_freq = frequencies.at(victim_page);
        int victim_index = 0;

        for (size_t i = 1; i < frames.size(); ++i)
        {
            int candidate = frames[i];
            int freq = frequencies.at(candidate);
            if (freq < min_freq || (freq == min_freq && candidate < victim_page))
            {
                min_freq = freq;
                victim_page = candidate;
                victim_index = i;
            }
        }

        frames[victim_index] = page;
        frequencies.erase(victim_page);
        page_replacements++;
    }
...
public:
    Otimo(int n_frames) : num_frames(n_frames), page_replacements(0) {}
    int get_page_replacements() const { return page_replacements; }
...
protected:
    ...
    void replace_page(int page, const std::vector<int> &order_process, int current_index)
    {
        if (frames.empty())
            return;
        ...
        for (size_t i = 0; i < frames.size(); i++)
        {
            int current_page_in_frame = frames[i];
            int next_use = -1;
            for (size_t j = current_index + 1; j < order_process.size(); j++)
            {
                if (order_process[j] == current_page_in_frame)
                {
                    next_use = j;
                    break;
                }
...
public:
    void execute(const std::vector<int> &order_process)
    {
        for (size_t i = 0; i < order_process.size(); i++)
        {
            int page = order_process[i];
            if (!is_page_in_memory(page))
            {
                if (frames.size() >= (size_t)num_frames)
                {
                    replace_page(page, order_process, (int)i);
                }
                else
                {
                    frames.push_back(page);
